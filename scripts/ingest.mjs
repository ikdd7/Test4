...
function splitNotice(body) {
  // NFTC 소절 번호는 "1.x(일반사항)"·"2.x(기술기준)"뿐이고, 절 제목은 한글(또는 여는 괄호)로 시작한다.
  //  이 두 가드가 없으면 표 행("0.75 m 미만", "13.7 12.2 0.28")의 소수가 절 번호로 오인되어
  //  표가 쓰레기 청크로 쪼개지고 원래 절 본문이 표 앞에서 끊긴다(실측 50건). 비매칭 행은 직전 절에 자동 포함.
  const re = /(^|\n)[ \t]*([12]\.\d+)\s+(?=[가-힣(])/g;
  const marks = [];
  let m;
